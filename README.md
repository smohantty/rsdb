...
- `rsdbd`: the target-side daemon packaged for Tizen-style `systemd` + `rpm`
  environments.
- Shell-script entrypoints for release, live regression, development-device
  daemon refresh, and a local loopback harness.
...
## Install `rsdbd` From A Release Asset

Download the RPM that matches the device architecture:
...
- Refresh or publish the GitHub release:
